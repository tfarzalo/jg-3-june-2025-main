-- Fix the ambiguous column reference in rename_job_category function
...
DECLARE
  ...
  v_existing_id uuid;
BEGIN
  -- 1. Check user permissions (FIXED: Added table alias 'p')
  ...
  -- FIXED: Added table alias 'bc'
  UPDATE public.billing_categories bc
  SET name = p_new_name
  WHERE bc.name = v_old_name;

  -- 7. Update job_categories
  -- FIXED: Added table alias 'jc' and fully qualified column references
  UPDATE public.job_categories jc
  SET 
    name = p_new_name,
    updated_at = now()
  WHERE jc.id = p_category_id;
...
WHERE n.nspname = 'public' AND p.proname = 'rename_job_category';
