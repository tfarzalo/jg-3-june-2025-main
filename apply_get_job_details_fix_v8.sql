-- Apply the get_job_details function fix - Version 8
-- This script fixes the syntax error by separating nested JSON objects into variables

-- Drop and recreate the function with the corrected structure
...
BEGIN
    ...
    v_extra_charges_details := CASE 
        WHEN v_extra_charges_hours > 0 THEN
            json_build_object(
                ...
                'hourly_rate', COALESCE(v_hourly_bill_amount, 40), -- Default to $40/hr if no hourly billing configured
                'sub_pay_rate', COALESCE(v_hourly_sub_pay_amount, 25), -- Default to $25/hr if no hourly billing configured
    ...
    -- Get work order data separately to avoid syntax issues
