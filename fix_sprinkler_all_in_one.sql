-- Single All-in-One Fix for Sprinkler Image Support
-- This script fixes the "cannot change return type" error by explicitly dropping functions first.

-- 1. DROP the existing function to avoid return type conflict
DROP FUNCTION IF EXISTS create_work_order_folder_structure(uuid, text, text, uuid);

-- 2. RECREATE create_work_order_folder_structure with Sprinkler Images support
...
-- 3. DROP get_upload_folder to ensure clean update
DROP FUNCTION IF EXISTS get_upload_folder(uuid, uuid, text);

-- 4. RECREATE get_upload_folder with Sprinkler Images support
...
-- 5. Verification
SELECT 'Sprinkler support functions updated successfully' as status;
