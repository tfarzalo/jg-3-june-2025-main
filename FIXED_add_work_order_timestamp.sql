-- QUICK FIX - Correct column names and add timestamp fields
-- This fixes the column name errors from the previous version
...
BEGIN
    ...
    -- Get work order data with timestamp and submitted_by (FIXED VERSION)
    SELECT json_build_object(
        'id', wo.id,
        'submission_date', wo.submission_date,
        'created_at', wo.created_at,
        'submitted_by_name', COALESCE(u.full_name, u.email, 'System'),
    ...
    -- Get unit size data (FIXED: use correct column name)
    SELECT json_build_object(
        'id', us.id,
    ...
    -- Get job type data (FIXED: use correct column name)
    SELECT json_build_object(
        'id', jt.id,
    ...
    -- Get job phase data (FIXED: use correct column names and join)
    SELECT json_build_object(
        'id', jp.id,
    ...
    -- Build the final result (FIXED: match v8 structure)
    SELECT json_build_object(
        'id', j.id,
