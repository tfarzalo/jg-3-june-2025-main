#!/bin/bash
# Fix and redeploy calendar feed with proper code

cat > supabase/functions/calendar-feed/index.ts << 'EOFINDEX'
...
const buildJobTitle = (job: any, property: any, assigneeName?: string, needsAssignment?: boolean) => {
  ...
  const streetAddr = formatStreetAddress(property);
  const unit = job.unit_number || job.unit;
  if (streetAddr && unit) {
    parts.push(`${streetAddr} Unit ${unit}`);
  } else if (streetAddr) {
    ...
    if (scope === "events_and_job_requests") {
      ...
      if (jobs && jobs.length > 0) {
        ...
        const { data: properties } = propertyIds.length > 0 ? await supabase
          .from("properties")
          .select("id, property_name, name, address, address_1, address_2, city, state, postal_code, zip")
    ...
    if (scope === "completed_jobs") {
      const { data: jobs, error: jErr } = await supabase
        ...
        .select(`
          ...
          job_type:job_types(job_type_label),
          unit_number, unit, property_id, assigned_to,
          scheduled_date, status, current_phase_id,
...
echo "File written. Now deploying..."
