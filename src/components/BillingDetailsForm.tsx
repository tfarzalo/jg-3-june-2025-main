...
export function BillingDetailsForm() {
  ...
  const [showNewUnitSizeModal, setShowNewUnitSizeModal] = useState(false);
  const [newUnitSizeLabel, setNewUnitSizeLabel] = useState('');
  const [newUnitSizeTarget, setNewUnitSizeTarget] = useState<{ propertyBillingCategoryId: string; lineItemId: string } | null>(null);
  ...
  const handleAddNewUnitSize = async () => {
    if (!newUnitSizeLabel.trim() || !newUnitSizeTarget) return;
    ...
    try {
      ...
      const { data: existing, error: checkError } = await supabase
        .from('unit_sizes')
        .select('id')
        .eq('unit_size_label', newUnitSizeLabel.trim())
        .single();

      if (checkError && checkError.code !== 'PGRST116') throw checkError;
      if (existing) throw new Error('This unit size already exists.');

      const { data: newUnitSize, error: insertError } = await supabase
        .from('unit_sizes')
        .insert({ unit_size_label: newUnitSizeLabel.trim() })
        .select()
        .single();

      if (insertError) throw insertError;

      // Manually add to state to trigger re-render before full fetch
      setUnitSizes(prev => [...prev, newUnitSize].sort((a, b) => a.unit_size_label.localeCompare(b.unit_size_label)));

      handleLineItemChange(
        newUnitSizeTarget.propertyBillingCategoryId,
        newUnitSizeTarget.lineItemId,
        'unitSizeId',
        newUnitSize.id
      );

      setShowNewUnitSizeModal(false);
      setNewUnitSizeLabel('');
      setNewUnitSizeTarget(null);

    } catch (err) {
      console.error('Error adding new unit size:', err);
      setError(err instanceof Error ? err.message : 'Failed to add new unit size.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAddNewCategory = async (name: string, description: string) => {
    ...
    // Simple session check using the auth context
    if (!session) {
    ...
    const timeoutId = setTimeout(() => {
      console.error('Function timeout - operation took too long');
      setError('Operation timed out. Please try again.');
      setLoading(false);
    }, 30000); // 30 second timeout

    try {
      console.log('Adding new category:', { name, description, propertyId });

      // Check if category name already exists in job_categories
      console.log('Step 1: Checking for existing job category...');
      const { data: existingJobCategory, error: checkError } = await supabase
        .from('job_categories')
        .select('id, name')
        .eq('name', name)
        .single();

      console.log('Step 2: Existing job category check result:', { existingJobCategory, checkError });

      if (checkError && checkError.code !== 'PGRST116') {
        console.error('Error checking for existing job category:', checkError);
        throw checkError;
      }

      if (existingJobCategory) {
        console.log('Step 3: Job category already exists, using existing:', existingJobCategory);
        // Use existing job category instead of creating a new one
        
        // Check if this property already has this category
        console.log('Step 4: Checking for existing property category...');
        const { data: existingPropertyCategory, error: propCheckError } = await supabase
          .from('billing_categories')
          .select('id')
          .eq('property_id', propertyId)
          .eq('name', name)
        ...
        console.log('Step 5: Existing property category check result:', { existingPropertyCategory, propCheckError });

        if (propCheckError && propCheckError.code !== 'PGRST116') {
          console.error('Error checking for existing property category:', propCheckError);
          throw propCheckError;
        }

        if (existingPropertyCategory) {
          throw new Error('This category already exists for this property');
        }

        // Create only the property billing category
        console.log('Step 6: Creating property billing category...');
          ...
          .insert([
            {
              ...
              sort_order: propertyBillingCategories.length + 1
            }
          ])
          .select()
          .single();

        console.log('Step 7: Property billing category creation result:', { newPropertyBillingCategory, billingCategoryError });

        if (billingCategoryError) {
          console.error('Error creating property billing category:', {
            error: billingCategoryError,
            details: billingCategoryError.details,
            hint: billingCategoryError.hint,
            code: billingCategoryError.code
          });
          throw billingCategoryError;
        }

        console.log('Step 8: Property billing category created successfully:', newPropertyBillingCategory);
        ...
        console.log('Step 9: Refreshing property billing data...');
        // Refresh the data
        await fetchPropertyBillingData();
        
        console.log('Step 10: Cleaning up form...');
        setShowNewCategoryModal(false);
        setNewCategoryName('');
        setNewCategoryDescription('');
        setHasChanges(true);

        console.log('Step 11: Category addition completed successfully');

      } else {
        // Create new job category first
        console.log('Step 12: Creating new job category...');
        ...
        console.log('Step 13: Job category creation result:', { newJobCategory, jobCategoryError });

        if (jobCategoryError) {
          console.error('Error creating job category:', {
            error: jobCategoryError,
            details: jobCategoryError.details,
            hint: jobCategoryError.hint,
            code: jobCategoryError.code
          });
          
          // Handle specific error types
          if (jobCategoryError.code === '23505') {
            throw new Error('A category with this name already exists');
          } else if (jobCategoryError.code === '42501') {
            throw new Error('Permission denied. You may not have access to create categories.');
          } else {
            throw jobCategoryError;
          }
        }

        console.log('Step 14: Job category created successfully:', newJobCategory);

        // Then create the property billing category
        console.log('Step 15: Creating property billing category...');
        const { data: newPropertyBillingCategory, error: billingCategoryError } = await supabase
          .from('billing_categories')
          .insert([
            {
        ...
        console.log('Step 16: Property billing category creation result:', { newPropertyBillingCategory, billingCategoryError });
        ...
        console.log('Step 17: Property billing category created successfully:', newPropertyBillingCategory);
        ...
        console.log('Step 18: Refreshing job categories...');
        // Refresh the data
        await fetchJobCategories();
        console.log('Step 19: Refreshing property billing data...');
        await fetchPropertyBillingData();
        
        console.log('Step 20: Cleaning up form...');
        setShowNewCategoryModal(false);
        setNewCategoryName('');
        setNewCategoryDescription('');
        setHasChanges(true);

        console.log('Step 21: Category addition completed successfully');
      }

    } catch (err) {
      console.error('Error in handleAddNewCategory (caught in try/catch):', err);
      const errorMessage = err instanceof Error ? err.message : 'Failed to add new category.';
                      ...
                      <div key={lineItem.id} className="flex items-center space-x-4">
                        {pbc.name !== 'Extra Charges' ? (
                          <div className="flex items-center space-x-2 flex-1">
                            ...
                            <button
                              onClick={() => {
                                setNewUnitSizeTarget({ propertyBillingCategoryId, lineItemId: lineItem.id });
                                setShowNewUnitSizeModal(true);
                              }}
                              className="p-2 text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300"
                              title="Add new unit size"
                            >
        ...
        {showNewUnitSizeModal && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4">
            <div className="bg-white dark:bg-[#1E293B] rounded-lg p-6 w-full max-w-md">
              ...
              <div className="space-y-4">
                <div>
                  <label htmlFor="newUnitSizeLabel" className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-2">
                    Unit Size Label *
                  </label>
                  <input
                    type="text"
                    id="newUnitSizeLabel"
                    value={newUnitSizeLabel}
                    onChange={(e) => setNewUnitSizeLabel(e.target.value)}
                    className="w-full h-11 px-4 bg-gray-50 dark:bg-[#0F172A] border border-gray-300 dark:border-[#2D3B4E] rounded-lg text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
              ...
              <div className="mt-6 flex justify-end gap-2">
                <button
                  type="button"
                  onClick={() => setShowNewUnitSizeModal(false)}
                  className="px-4 py-2 text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-500"
                >
                  Cancel
                </button>
                <button
                  type="button"
                  onClick={handleAddNewUnitSize}
                  disabled={!newUnitSizeLabel.trim() || loading}
                  className="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {loading ? 'Saving...' : 'Save Unit Size'}
                </button>
              </div>
            </div>
          </div>
