...
export function UserLoginAlertManager() {
  ...
  // Initialize with current user to prevent self-notification on mount
  useEffect(() => {
    if (currentUser?.id && !isInitialized.current) {
      previousOnlineUsers.current.add(currentUser.id);
