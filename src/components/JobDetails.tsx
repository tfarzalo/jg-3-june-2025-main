...
export function JobDetails() {
  ...
  useEffect(() => {
    const fetchWorkOrderFolderId = async () => {
      if (!job?.work_order || !job?.property?.name) {
                        ...
                        <>
                          ...
                          {(isAdmin || isJGManagement) && (
                            <button
                              ...
                              className="inline-flex items-center px-3 py-1.5 bg-orange-600 hover:bg-orange-700 text-white text-sm font-medium rounded-lg transition-colors border border-orange-300"
                              title="Admin Override: Bypass email and approve directly"
                            >
              ...
              <div className="mb-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 bg-gray-50 dark:bg-[#0F172A] p-4 rounded-lg">
                  <div className={`p-3 rounded-lg border ${accentWallDisplayLabel ? 'border-green-500/50 bg-green-50 dark:bg-green-900/30' : 'border-gray-200 dark:border-gray-700'} flex flex-col justify-center`}>
                    ...
                    <p className={`text-lg font-bold ${accentWallDisplayLabel ? 'text-green-800 dark:text-green-200' : ''} mt-1`}>
                  ...
                  <div className={`p-3 rounded-lg border ${(job?.work_order?.accent_wall_count ?? 0) > 0 ? 'border-green-500/50 bg-green-50 dark:bg-green-900/30' : 'border-gray-200 dark:border-gray-700'} flex flex-col justify-center`}>
                    ...
                    <p className={`text-lg font-bold ${(job?.work_order?.accent_wall_count ?? 0) > 0 ? 'text-green-800 dark:text-green-200' : ''} mt-1`}>
            ...
            {FLAGS.BILLING_V2 ? (
              <BillingBreakdownV2 
                billing={{
                  ...
                  extra_charges_details: job.extra_charges_details,
                      ...
                      <div className="flex justify-between items-center p-3 bg-gray-50 dark:bg-[#0F172A] rounded-lg">
                        <div className="flex-1">
                          ...
                          <span className="text-sm text-gray-500 dark:text-gray-400 ml-2">
                            ({formatCurrency(job.extra_charges_details?.hourly_rate || 0)}/hr)
                          </span>
                        </div>
                        <div className="text-right">
                          <div className="text-sm text-gray-600 dark:text-gray-400">
                            {extraHours} hrs × {formatCurrency(job.extra_charges_details?.hourly_rate || 0)}
                          </div>
                          <div className="font-semibold text-gray-900 dark:text-white">
                            {formatCurrency(extraChargesAmount)}
                          </div>
