  ...
  return (
    <div className={`flex items-center space-x-3 ${className}`}>
      <div className={`relative flex-shrink-0 ${sizeClasses[size]} rounded-full overflow-hidden border-2 ${borderColor} shadow-sm`}>
        ...
        {/* Fallback avatar with initials - shown if no avatar or if image fails to load */}
        <div className={`${sizeClasses[size]} bg-blue-600 rounded-full flex items-center justify-center text-white font-semibold ${avatarUrl ? 'hidden' : ''}`}>
          {initials}
        </div>
