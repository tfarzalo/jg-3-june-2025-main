...
export function WhatsNewModal({
  ...
  const renderUpdateCard = (entry: WhatsNewEntry) => {
    ...
    return (
      <div
        key={entry.id}
        className="flex min-h-[138px] items-start gap-4 rounded-[24px] border border-slate-200/90 bg-slate-50/90 px-5 py-5 transition dark:border-slate-700/60 dark:bg-slate-900/50"
      >
        <div className={`inline-flex h-14 w-14 flex-shrink-0 items-center justify-center rounded-[18px] text-white ${accent.pill}`}>
        ...
        <div className="flex min-h-[92px] min-w-0 flex-1 flex-col justify-between">
          <div>
            <div className="flex flex-wrap items-center gap-3">
              <h3 className="text-[17px] font-semibold leading-tight text-slate-800 dark:text-slate-100">
                {entry.title}
              </h3>
              {entry.badge_label && (
                <span className={`rounded-full px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.08em] ${getWhatsNewBadgeClasses(entry.badge_label)}`}>
                  {entry.badge_label}
                </span>
              )}
            </div>
            ...
            <p
              className="mt-3 text-[14px] leading-7 text-slate-500 dark:text-slate-300"
              style={{
                display: '-webkit-box',
                WebkitLineClamp: 3,
      ...
      <div className="relative z-10 w-full max-w-4xl overflow-hidden rounded-[28px] border border-slate-200/80 bg-white shadow-[0_36px_100px_rgba(15,23,42,0.32)] dark:border-slate-700/60 dark:bg-[#0F172A]">
        ...
        <div className="max-h-[74vh] overflow-y-auto px-4 py-4 md:px-5 md:py-5">
          <div className="grid gap-4">
            {effectiveEntries.map((entry) => renderUpdateCard(entry))}
          </div>
        </div>
      </div>
    </div>
