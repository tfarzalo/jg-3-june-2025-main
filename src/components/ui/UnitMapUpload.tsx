...
export function UnitMapUpload({
  ...
  useEffect(() => {
    const generatePreviewUrl = async () => {
      if (currentFilePath) {
        try {
          console.log('Attempting to load unit map with path:', currentFilePath);
          const previewResult = await getPreviewUrl(supabase, 'files', currentFilePath);
          setPreviewUrl(previewResult.url);
        } catch (error) {
          console.error('Error generating preview URL for existing file:', error);
          
          // Try alternative path formats
          const alternativePaths = [
            currentFilePath,
            currentFilePath.replace(/ /g, '_'), // Replace spaces with underscores
            currentFilePath.replace(/ /g, '%20'), // URL encode spaces
          ];
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      ...
      if (result.success && result.filePath) {
        ...
        try {
          const previewResult = await getPreviewUrl(supabase, 'files', result.filePath);
          setPreviewUrl(previewResult.url);
        } catch (error) {
          console.error('Error generating preview URL for uploaded file:', error);
        ...
        <div className="relative border border-gray-200 dark:border-gray-700 rounded-lg p-4">
          <div className="flex items-center space-x-3">
            {previewUrl ? (
              <img
                src={previewUrl}
                alt="Unit Map Preview"
                className="h-8 w-8 object-cover rounded"
                onError={(e) => {
                  console.error('Failed to load unit map image:', previewUrl);
