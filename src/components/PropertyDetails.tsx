...
export function PropertyDetails() {
  ...
  console.log('PropertyDetails: Received ID from URL params:', propertyId);
  ...
  const { jobRequests: phaseJobRequests, workOrders, completed, cancelled, loading: phaseLoading } = usePropertyPhaseCounts(propertyId);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (propertyError) {
          console.error('PropertyDetails: Error fetching property:', propertyError);
          throw propertyError;
        }
        if (!propertyData) {
  ...
  return (
    <div className="p-6 bg-gray-100 dark:bg-[#0F172A] min-h-screen smooth-scroll">
          ...
          {property.unit_map_file_path ? (
            <div className="bg-white dark:bg-[#1E293B] rounded-xl p-6 shadow-lg border border-gray-100 dark:border-gray-800">
              ...
              <div className="relative w-full h-64 bg-gray-100 dark:bg-gray-800 rounded-lg overflow-hidden group cursor-pointer" style={{ pointerEvents: 'auto' }}>
                <img 
                  src={`${import.meta.env.VITE_SUPABASE_URL}/storage/v1/object/public/files/${property.unit_map_file_path}`}
                    ...
                    <div className="space-y-2">
                      {categoryDetails[category.id]?.map(detail => (
                        <div key={detail.unit_size_id} className="flex justify-between items-center text-sm py-2 px-3 bg-white dark:bg-gray-800 rounded-md">
                          <span className="text-gray-600 dark:text-gray-400 font-medium">{unitSizes[detail.unit_size_id]}</span>
      ...
      <Lightbox
        ...
        imageUrl={property?.unit_map_file_path ? `${import.meta.env.VITE_SUPABASE_URL}/storage/v1/object/public/files/${property.unit_map_file_path}` : ''}
