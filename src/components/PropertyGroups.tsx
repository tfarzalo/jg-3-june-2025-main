...
export function PropertyGroups() {
  ...
  const handleArchiveSelected = async () => {
    ...
    setProcessing(true);
    try {
      // Update property groups to set is_archived to true
      ...
      toast.success(`Successfully archived ${selectedGroups.length} ${selectedGroups.length !== 1 ? 'property groups' : 'property group'}`);
      
      // Refresh the property group list
      fetchPropertyGroups();
    } catch (err) {
      console.error('Error archiving property groups:', err);
      toast.error('Failed to archive property groups. Please try again.');
    } finally {
      setProcessing(false);
    }
  };
  ...
  return (
    <div className="p-6 bg-gray-100 dark:bg-[#0F172A] min-h-screen">
        ...
        <div className="flex space-x-3">
          {selectedGroups.length > 0 && (
            ...
            >
              ...
              Archive Selected ({selectedGroups.length})
            </button>
          )}
          <button
            onClick={() => navigate('/dashboard/property-groups/new')}
            className="inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg transition-colors"
          >
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-[#2D3B4E]">
              {filteredGroups.map((group) => (
                <tr 
                  ...
                  onClick={() => navigate(`/dashboard/property-groups/${group.id}`)}
                >
      ...
      {showArchiveConfirm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white dark:bg-[#1E293B] rounded-lg p-6 max-w-md w-full shadow-xl">
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Archive Selected Property Groups</h3>
            <p className="text-gray-700 dark:text-gray-300 mb-4">
              Are you sure you want to archive {selectedGroups.length} selected {selectedGroups.length !== 1 ? 'property groups' : 'property group'}?
              Archived property groups will be moved to the Archives section.
            </p>
            <div className="flex justify-end space-x-3">
              ...
              >
                {processing ? 'Archiving...' : 'Archive Groups'}
              </button>
            </div>
          </div>
        </div>
      ...
      <div className="mt-6 flex justify-end">
        <button
          onClick={() => navigate('/dashboard/property-groups/archives')}
