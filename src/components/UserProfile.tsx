...
export function UserProfile() {
  ...
  const fetchUserProfile = async () => {
    try {
      ...
      console.log('UserProfile: User ID from auth:', userData.user.id);
      console.log('UserProfile: User object:', userData.user);
      
      // Check if user ID has any unexpected characters
      let userId = userData.user.id;
      if (userId && userId.includes(':')) {
        console.error('UserProfile: User ID contains unexpected characters:', userId);
        // Clean the user ID by removing anything after the first colon
        userId = userId.split(':')[0];
        console.log('UserProfile: Cleaned user ID:', userId);
      } else {
        console.log('UserProfile: User authenticated with clean ID:', userId);
      }
      ...
      if (error) {
        ...
        if (error.code === 'PGRST116') {
          ...
          const newProfile = {
            id: userId, // Use the cleaned user ID
