...
export function ChatDock() {
  ...
  // Load chat titles and user info
  useEffect(() => {
    const loadChatInfo = async () => {
      ...
      try {
        ...
        // Load each conversation's data to get proper participant information
        ...
        <div className="flex items-center space-x-2">
          {openChats.map(chat => {
            ...
            return (
              <div
                key={chat.id}
                className={`relative flex items-center space-x-2 rounded-lg px-3 py-1.5 min-w-0 cursor-pointer transition-all duration-300 ${
                  chat.unread > 0 
                    ...
                    return avatarProps.avatarUrl ? (
                      <img
                        ...
                        onError={(e) => {
                          ...
                          if (parent) {
                            parent.innerHTML = `<span class="text-xs font-medium text-gray-600 dark:text-gray-300">${avatarProps.initials}</span>`;
                          }
                        }}
                      />
                    ) : (
