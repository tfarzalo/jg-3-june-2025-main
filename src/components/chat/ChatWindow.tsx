...
export function ChatWindow({ conversationId, currentUserId }: ChatWindowProps) {
  const { closeChat, toggleMinimize, setUnread } = useChatTray();
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        async (payload) => {
          console.log('New message received in ChatWindow:', payload);
          ...
          if (senderData) {
            newMessage.sender = senderData as User;
          }
          ...
          // Reset unread count since window is open
          setUnread(conversationId, 0);
        }
      )
      .subscribe();
