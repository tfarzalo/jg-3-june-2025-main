  ...
  return (
    <div className="p-6 bg-gray-100 dark:bg-[#0F172A] min-h-screen">
      ...
      <div className="space-y-6">
          ...
          <div className="relative">
            ...
            {showFilters && (
              <div className="absolute right-0 mt-2 w-64 bg-white dark:bg-[#1E293B] rounded-lg shadow-lg z-10 border border-gray-200 dark:border-[#2D3B4E]">
                <div className="p-4">
                  <h3 className="text-sm font-medium text-gray-900 dark:text-white mb-3">User Roles</h3>
                  <div className="space-y-2">
                    <label className="flex items-center">
                      ...
                      <span className="ml-2 text-gray-700 dark:text-gray-300 flex items-center">
                        <ShieldCheck className="h-4 w-4 mr-1 text-red-600 dark:text-red-400" />
                        Admin
                      </span>
                    </label>
                    <label className="flex items-center">
                      <input
                        type="checkbox"
                        checked={roleFilter.includes('jg_management')}
                        onChange={() => toggleRoleFilter('jg_management')}
                        className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 dark:border-gray-600 rounded"
                      />
                      <span className="ml-2 text-gray-700 dark:text-gray-300 flex items-center">
                        <UserCog className="h-4 w-4 mr-1 text-purple-600 dark:text-purple-400" />
                        JG Management
                      </span>
                    </label>
                    <label className="flex items-center">
                      <input
                        type="checkbox"
                        checked={roleFilter.includes('subcontractor')}
                        onChange={() => toggleRoleFilter('subcontractor')}
                        className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 dark:border-gray-600 rounded"
                      />
                      <span className="ml-2 text-gray-700 dark:text-gray-300 flex items-center">
        ...
        {/* Role filter indicators */}
        {roleFilter.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-4">
            {roleFilter.map(role => (
              <div 
                key={role}
                className={`flex items-center px-3 py-1 rounded-full text-sm ${getRoleColor(role)}`}
              >
                {getRoleIcon(role)}
                <span>{role === 'jg_management' ? 'JG Management' : role.charAt(0).toUpperCase() + role.slice(1)}</span>
                <button 
