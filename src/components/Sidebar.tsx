...
function LiveStatusBadge({ isCollapsed }: { isCollapsed: boolean }) {
  ...
  // When collapsed, show just a green dot
  if (isCollapsed) {
    return (
      <div 
...
export function Sidebar() {
  ...
  const getIconColor = (label: string) => {
    switch (label) {
      ...
      case 'Messaging':
        return '#1E40AF'; // Blue (complementary to Users)
      ...
      // SUPPORT
      case 'Support':
        return '#F59E0B'; // Amber/Orange (helpful, friendly tone)
    ...
    if (isSubcontractor) {
      return [
        ...
        },
        {
          title: 'SETTINGS',
          items: [
            { icon: Settings, label: 'Admin Settings', to: '/dashboard/settings' },
          ]
        }
      ];
    }
  ...
  return (
    <div className={`${isCollapsed ? 'w-20' : 'w-64'} h-screen bg-white dark:bg-[#0F172A] border-r border-gray-200 dark:border-[#1E293B] flex flex-col transition-all duration-300`}>
      ...
      <div className="p-4 border-t border-gray-200 dark:border-[#1E293B]">
        ...
        >
          <HelpCircle 
            className={`${isCollapsed ? 'h-6 w-6' : 'h-5 w-5 mr-3'}`}
            style={{ color: getIconColor('Support') }}
          />
          {!isCollapsed && 'Support'}
        </NavLink>
        <button 
