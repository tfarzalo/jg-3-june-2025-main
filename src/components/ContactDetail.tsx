...
export function ContactDetail() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'notes' | 'communications'>('overview');
  ...
  const fetchContactDetails = async () => {
    try {
      ...
      const contactData: Contact = {
        ...
        property_group: data.property_group || '',
      ...
      setFormData({
        ...
        property_group: contactData.property_group,
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .update({
          ...
          property_group: formData.property_group,
  ...
  const handleCreateProperty = async () => {
    if (!formData.property_name || !formData.property_address) {
    ...
    try {
      ...
      const { data, error } = await supabase.rpc('create_property_from_contact', {
        contact_id_param: contactId,
        property_name_param: formData.property_name,
        property_address_param: formData.property_address,
        property_group_param: formData.property_group || null
      });

      if (error) throw error;

      toast.success('Property created and linked to contact successfully');
      
      // Refresh contact data to show the new property link
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                {editing ? (
                  ...
                  <p className="text-gray-900 dark:text-white">{contact.property_address || 'No property address'}</p>
                )}
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                ...
                {editing ? (
                  ...
                  <p className="text-gray-900 dark:text-white">{contact.property_group || 'No property group'}</p>
                )}
              </div>
              {editing && formData.property_name && formData.property_address && (
                <div className="pt-2">
                  <button
                    type="button"
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                    This will create a property in the system and link it to this contact
                  </p>
                </div>
              )}
              <div>
