...
export function PropertyMap({ address, className = '' }: PropertyMapProps) {
  ...
  useEffect(() => {
    ...
    const geocodeAddress = async () => {
      try {
        ...
        if (isMounted.current && data && data.length > 0) {
          ...
          // Guard: only set view if map is fully initialized and loaded
          const map = mapInstanceRef.current;
          if (!isNaN(latNum) && !isNaN(lonNum) && map && (map as any)._loaded) {
    ...
    // Also try geocoding immediately in case map is already loaded
