...
function MessagingPage() {
  ...
  const { openChat, openChats, setTitle } = useChatTray();
  ...
  const getConversationLastMessage = (conversation: Conversation) => {
    ...
    const truncatedBody = lastMsg.body.length > 50 ? lastMsg.body.substring(0, 50) + '...' : lastMsg.body;
  ...
  // Helper function to select a conversation and open it in the chat tray
  ...
  return (
    <div className="flex h-screen bg-gray-100 dark:bg-gray-900">
            ...
            <div className="p-2">
              {conversations.length === 0 ? (
                <div className="text-center py-12 px-4 text-gray-500 dark:text-gray-400">
                  ...
                  <p className="text-sm mb-4">Start a conversation to begin messaging with your team.</p>
                  <button
                    onClick={() => setActiveTab('users')}
      ...
      <div className={`flex-1 flex flex-col ${selectedConversation ? 'lg:flex' : 'hidden lg:flex'}`}>
        {selectedConversation ? (
          <div key={selectedConversation.id} className="flex-1 flex flex-col h-full">
            {/* Chat Header - Sticky */}
            <div className="sticky top-0 z-20 p-3 sm:p-4 border-b border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 shadow-sm">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  <div className="flex items-center space-x-3">
                    {(() => {
                      ...
                      return (
                        <>
                          {conversationUser?.avatar_url ? (
                            ...
                            <div className="w-10 h-10 rounded-full flex items-center justify-center text-base font-semibold bg-blue-500 dark:bg-blue-600 text-white">
                              {(conversationUser?.full_name || conversationUser?.email || 'U').charAt(0).toUpperCase()}
                            </div>
                          )}
                          <div>
                            ...
                            {lastMessages[selectedConversation.id] && (
                              <div className="flex items-center space-x-1 text-xs text-gray-400 dark:text-gray-500 mt-1">
                                <Clock className="h-3 w-3" />
                                <span>Last message: {formatFullTimestamp(lastMessages[selectedConversation.id].created_at)}</span>
                              </div>
                            )}
                          </div>
                        </>
                        ...
                        <div key={message.id} className={`flex items-start space-x-3 ${isOwnMessage ? 'flex-row-reverse space-x-reverse' : ''} transition-all duration-200 ease-in-out`}>
                          ...
                          <div className="flex-shrink-0">
                            {message.sender?.avatar_url ? (
                              ...
                              <div className="w-8 h-8 rounded-full flex items-center justify-center text-sm font-semibold bg-blue-500 dark:bg-blue-600 text-white">
            ...
            <div className="sticky bottom-0 z-10 p-4 border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm">
              <div className="flex items-center space-x-3">
                <input
                  ...
                  className="flex-1 px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-400"
                />
                <button
                  onClick={sendMessage}
