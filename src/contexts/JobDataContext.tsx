...
export function JobDataProvider({ children }: { children: React.ReactNode }) {
    ...
    for (const phase of phases) {
      ...
      try {
        ...
        if (phase === 'Job Request') {
          ...
          ascending = false;
        } else if (['Work Order', 'Pending Work Order', 'Completed Work Orders', 'Grading', 'Quality Control', 'Invoicing'].includes(phase)) {
  ...
  const preloadCommonPhases = useCallback(async () => {
    ...
    const commonPhases = ['Job Request', 'Pending Work Order', 'Work Order', 'Completed Work Orders', 'Quality Control', 'Completed', 'Invoicing'];
