        ...
        for (const j of jobs) {
          try {
            ...
            if (isNaN(startDate.getTime())) {
              console.log(`Skipping job ${j.id}: invalid scheduled_date ${j.scheduled_date}`);
              continue;
            }
      ...
      if (completed.length > 0) {
        ...
        for (const j of completed) {
          try {
            ...
            if (isNaN(startDate.getTime())) {
              console.log(`Skipping completed job ${j.id}: invalid date ${scheduledDate}`);
              continue;
            }
            startDate.setHours(0, 1, 0, 0); // 12:01 AM
      ...
      if (jobs && jobs.length > 0) {
        ...
        for (const j of jobs) {
          try {
            ...
            const startDate = new Date(j.scheduled_date);
            if (isNaN(startDate.getTime())) {
              console.log(`Skipping subcontractor job ${j.id}: invalid date ${j.scheduled_date}`);
              continue;
            }
            startDate.setHours(0, 1, 0, 0); // 12:01 AM
            ...
            // For subcontractor feeds, don't include assignee name in title since it's their own feed
            const title = jobSummary(j, property) || `Job #${j.id}`;
