/*
  # Add Job Phase Relationship to Jobs Table

  1. Changes
    - Add current_phase_id column to jobs table
    - Add foreign key constraint to job_phases table
    - Add index for better query performance
    - Update create_job function to set current_phase_id
...
ALTER TABLE jobs ADD COLUMN current_phase_id uuid REFERENCES job_phases(id);

-- Add index for better query performance
CREATE INDEX idx_jobs_current_phase_id ON jobs(current_phase_id);

-- Update the create_job function to handle current_phase_id
