-- Add missing ceiling and accent wall fields to get_job_details function
-- This migration adds the new fields that were added to work_orders table

-- Drop and recreate the function with the new fields
...
BEGIN
    ...
    v_unit_size_exists := EXISTS (
        SELECT 1 FROM unit_sizes 
        WHERE id = v_unit_size_id
    );

    -- Get the billing category ID for regular billing
    SELECT id INTO v_billing_category_id
    FROM billing_categories
    WHERE name = v_job_category_name
    AND property_id = v_property_id;

    -- Get the billing category ID for hourly billing (Extra Charges)
