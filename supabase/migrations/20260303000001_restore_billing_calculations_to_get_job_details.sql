/*
  Restore comprehensive billing calculations to get_job_details function
  
  This migration restores the billing calculation logic that was removed by
  20260225000005_update_get_job_details_with_is_occupied.sql
  
  The function now returns:
  - billing_details (regular billing from property rates)
  - hourly_billing_details (hourly billing rates)
  - extra_charges_details (extra charges calculations)
  - debug_billing_joins (for troubleshooting)
  
  While maintaining recent additions:
  - is_occupied field
  - is_full_paint field
*/
...
BEGIN
    ...
    -- Get work order data with is_occupied and is_full_paint fields
    ...
    -- Get job phase data with colors
    SELECT json_build_object(
        'id', jp.id,
    ...
    -- Build the final result including billing data
    SELECT json_build_object(
        'id', j.id,
