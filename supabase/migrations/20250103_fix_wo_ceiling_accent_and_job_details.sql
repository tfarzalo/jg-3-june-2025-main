-- 1) Normalize 0/NULL semantics and guard negative counts
ALTER TABLE public.work_orders
  ALTER COLUMN individual_ceiling_count DROP DEFAULT,
  ALTER COLUMN accent_wall_count DROP DEFAULT;

UPDATE public.work_orders SET individual_ceiling_count = NULL WHERE individual_ceiling_count = 0;
UPDATE public.work_orders SET accent_wall_count = NULL WHERE accent_wall_count = 0;
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE conname = 'chk_individual_ceiling_nonneg'
  ) THEN
    ALTER TABLE public.work_orders
      ADD CONSTRAINT chk_individual_ceiling_nonneg
      CHECK (individual_ceiling_count IS NULL OR individual_ceiling_count >= 0);
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'chk_accent_wall_nonneg'
  ) THEN
    ALTER TABLE public.work_orders
      ADD CONSTRAINT chk_accent_wall_nonneg
      CHECK (accent_wall_count IS NULL OR accent_wall_count >= 0);
  END IF;
END$$;

-- 2) Recreate get_job_details with backward-compatible fields and richer payload
...
BEGIN
  ...
  SELECT 
    wo.extra_hours, wo.extra_charges_description
  INTO 
    v_extra_charges_hours, v_extra_charges_description
