-- =====================================================
-- SAFE VERSION: process_decline_token
-- =====================================================
-- This version dynamically checks which columns exist
-- and only inserts into columns that are present
-- =====================================================
...
BEGIN
  ...
  -- Check if is_read column exists in user_notifications
  SELECT EXISTS (
  ...
  -- Create notifications with or without is_read column
  IF v_has_is_read THEN
    -- Version WITH is_read column
  ...
  ELSE
    -- Version WITHOUT is_read column (fallback)
    INSERT INTO user_notifications (
      user_id,
      title,
      message,
...
COMMENT ON FUNCTION public.process_decline_token(VARCHAR, TEXT) IS 
  'Processes decline action. Dynamically adapts to user_notifications table schema.';

-- Verify
SELECT 
  'Function created (schema-aware version)!' as status,
  routine_name
